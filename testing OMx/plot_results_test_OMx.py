...
def plot(skin_stim_time, names, voltages, g_exc, g_inh, spikes, step, save_to, plot_only=None):
	for name, voltage, g_e, g_i, s in zip(names, voltages, g_exc, g_inh, spikes):
		if plot_only and name != plot_only:
			continue
		if not("OM2" in name or "MN_E" in name or "MN_F" in name):
			continue
...
def run():
	...
	if len(sys.argv) == 2:
		...
		path = "/home/kseniia/Рабочий_стол/memristive-spinal-cord-master/GRAS/dat"

	plot(skin_stim_time, *read_data(path), step=step, save_to=f"{path}/OM2/results/", plot_only=nuclei) #must change path! /OMx/
